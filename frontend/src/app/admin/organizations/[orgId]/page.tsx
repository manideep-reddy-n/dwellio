...
type AdminTab = "overview" | "residents" | "blueprint" | "appeals";
...
export default function AdminOrganizationDetailPage() {
  ...
  const tabs: { id: AdminTab; label: string }[] = [
    ...
    { id: "residents", label: `Residents (${members.length})` },
  ...
  return (
    <div className="space-y-6">
          ...
          <Card className="lg:col-span-3">
            ...
            <CardContent>
              <div className="grid gap-2 sm:grid-cols-3">
                <StatPill label="Total members" value={members.length} />
                <StatPill
                  label="Residents"
                  value={members.filter((m) => m.roleName?.toLowerCase().includes("resident")).length}
                />
      ...
      {tab === "residents" && (
        <Card>
          <CardHeader>
            <CardTitle className="text-base">All members & allocations</CardTitle>
          </CardHeader>
                ...
                {members.map((m) => {
                  ...
                  return (
                    <tr key={m.id} className="border-b last:border-0">
                      <td className="py-2.5 pr-4 font-medium">{m.userFullName}</td>
                      <td className="py-2.5 pr-4 text-muted-foreground">{m.userEmail}</td>
                      <td className="py-2.5 pr-4">{m.roleName}</td>
