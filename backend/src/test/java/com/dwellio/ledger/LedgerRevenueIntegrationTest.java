...
@ActiveProfiles("test")
class LedgerRevenueIntegrationTest {
    ...
    @Test
    void chargePaymentLedgerAndRevenueDashboard() throws Exception {
        String ownerToken = registerAndLogin("owner-ledger@example.com", "Owner Ledger");
        String residentToken = registerAndLogin("resident-ledger@example.com", "Resident Ledger");

        String organizationId = createHostel(ownerToken);
        String roomId = createStructure(ownerToken, organizationId);
        String bedId = createBed(ownerToken, organizationId, roomId, "A");

        approveResident(ownerToken, residentToken, organizationId);
        String membershipId = getResidentMembershipId(ownerToken, organizationId, "resident-ledger@example.com");
        ...
        mockMvc.perform(post("/organizations/{organizationId}/occupancies/allocate", organizationId)
                        ...
                        .content("""
                                ...
                                """.formatted(membershipId, bedId)))
                .andExpect(status().isCreated());

        mockMvc.perform(get("/organizations/{organizationId}/payments", organizationId)
                        .header("Authorization", "Bearer " + ownerToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[?(@.chargeType == 'RENT')]").isNotEmpty());

        mockMvc.perform(get("/organizations/{organizationId}/ledger", organizationId)
                        .header("Authorization", "Bearer " + ownerToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].entryType").value("CHARGE_GENERATED"));

        mockMvc.perform(get("/organizations/{organizationId}/ledger/mine", organizationId)
                        .header("Authorization", "Bearer " + residentToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].entryType").value("CHARGE_GENERATED"));

        String paymentId = findRentPaymentId(ownerToken, organizationId);
        mockMvc.perform(patch("/organizations/{organizationId}/payments/{paymentId}", organizationId, paymentId)
                        .header("Authorization", "Bearer " + ownerToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {
        ...
        mockMvc.perform(get("/organizations/{organizationId}/ledger", organizationId)
                        ...
                        .header("Authorization", "Bearer " + ownerToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].entryType").value("PAYMENT_RECEIVED"));

        mockMvc.perform(post("/organizations/{organizationId}/metrics/rebuild", organizationId)
                        .header("Authorization", "Bearer " + ownerToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.expectedRevenueMonth").isNumber())
                .andExpect(jsonPath("$.collectedRevenueMonth").isNumber());
        ...
        assertThat(metrics.get("expectedRevenueMonth").asDouble()).isGreaterThan(0);
        assertThat(metrics.get("collectedRevenueMonth").asDouble()).isGreaterThan(0);
        assertThat(metrics.get("collectionRate").asDouble()).isGreaterThan(0);
    }

    private String findRentPaymentId(String ownerToken, String organizationId) throws Exception {
        MvcResult result = mockMvc.perform(get("/organizations/{organizationId}/payments", organizationId)
                        .header("Authorization", "Bearer " + ownerToken))
                .andExpect(status().isOk())
                .andReturn();

        for (JsonNode payment : objectMapper.readTree(result.getResponse().getContentAsString())) {
            if ("RENT".equals(payment.get("chargeType").asText())) {
                return payment.get("id").asText();
            }
        }
        throw new IllegalStateException("Rent payment not found");
    }
    ...
    private String createHostel(String ownerToken) throws Exception {
        MvcResult createResult = mockMvc.perform(post("/organizations")
                        ...
                        .content("""
                                {
                                  "name": "Ledger Hostel",
                                  "slug": "ledger-hostel-%s",
                                  "type": "HOSTEL",
                                  "hostelAudience": "CO_ED",
                                  "description": "Ledger revenue test",
                                  "city": "Hyderabad",
                                  "area": "Madhapur"
                                }
