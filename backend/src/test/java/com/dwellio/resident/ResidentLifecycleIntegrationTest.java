...
class ResidentLifecycleIntegrationTest {
    ...
    @Test
    void residentLifecycleProfileAggregatesMembershipFinancialAndHistory() throws Exception {
        String ownerToken = registerAndLogin("owner-life@example.com", "Owner Life");
        String residentToken = registerAndLogin("resident-life@example.com", "Resident Life");

        String organizationId = createHostel(ownerToken);
        approveResident(ownerToken, residentToken, organizationId);
        String membershipId = getResidentMembershipId(ownerToken, organizationId, "resident-life@example.com");
        ...
        mockMvc.perform(get("/organizations/{organizationId}/residents/{membershipId}/profile",
                        organizationId, membershipId)
                        .header("Authorization", "Bearer " + ownerToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.membership.id").value(membershipId))
                .andExpect(jsonPath("$.membership.userFullName").value("Resident Life"))
                .andExpect(jsonPath("$.currentOccupancy.bedLabel").value("A"))
                .andExpect(jsonPath("$.accommodationHistory.length()").value(1))
                .andExpect(jsonPath("$.financialSummary").exists())
                .andExpect(jsonPath("$.complaints.length()").value(1))
                .andExpect(jsonPath("$.complaintMetrics.totalComplaints").value(1))
                .andExpect(jsonPath("$.complaintMetrics.openComplaints").value(1));
    }
    ...
    private String createHostel(String ownerToken) throws Exception {
        MvcResult createResult = mockMvc.perform(post("/organizations")
                        ...
                        .content("""
                                {
                                  "name": "Lifecycle Hostel",
                                  "slug": "lifecycle-hostel-%s",
                                  "type": "HOSTEL",
